...
void main(void) // Fonction principale
{
    ...
    while (MyGame.CurrentRoom.Level <= 5) 
    {
        ...
        while (MyGame.Player.Stats.LP >= 0 && MyGame.CurrentRoom.Opponent.Stats.LP >= 0)
        {
            ...
            if (MyGame.CurrentRoom.Opponent.Stats.LP <= 0 && MyGame.CurrentRoom.Level == 5)
            {
                MyGame.CurrentRoom.DescriptionRoomOut(&MyGame);getchar();
                printf("merci d\'avoir joue au jeu");
                getchar();
                getchar();
                exit(404);
            }
            ...
            else if (MyGame.CurrentRoom.Opponent.Stats.LP <= 0)
            {
